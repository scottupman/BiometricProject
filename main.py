...
def modelC(xDark, yDark, X, y):
    X_train, X_test, y_train, y_test = train_test_split(X, y)
    xDark_train, xDark_test, yDark_train, yDark_test = train_test_split(xDark, yDark)
    
    df = SVC()
    df.fit(X_train, y_train)
    dfPre = df.predict(xDark_test)
    dAccScore = accuracy_score(yDark_test, dfPre)
    print("Accuracy: ", dAccScore)
...
# Bright, yBright = get_images.get_bright(image_directory)
